#!/usr/bin/env python3
"""
Test script for the improved parser with real PDF samples.
"""
...
def main():
    """
    Main function to test the improved parser with real PDF samples.
    """
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Test the improved parser with real PDF samples")
    parser.add_argument("--pdf", required=True, help="Path to the PDF file to parse")
    parser.add_argument("--out-dir", default="./out", help="Output directory for JSON and CSV files")
    ...
    cfg = Config(
        ...
        output={
            "json_path": os.path.join(args.out_dir, "arrests.json"),
            "csv_path": os.path.join(args.out_dir, "arrests.csv"),
            "ndjson_path": None,
            "pretty_json": True
        },
        logging={
    ...
    try:
        records = parse_pdf(args.pdf, cfg)
